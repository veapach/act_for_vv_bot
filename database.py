...
class Database:
    ...
    async def migrate_db(self):
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await self.check_and_add_column(
                db, "reports", "user_id", "INTEGER NOT NULL"
            )
            await self.check_and_add_column(
                db, "reports", "telegram_message_id", "INTEGER"
            )
    ...
    async def add_report(
        self, user_id: int, telegram_message_id: int, date: str, address: str
    ):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                ...
                VALUES (?, ?, ?, ?)
                """,
                (user_id, telegram_message_id, formatted_date, address),
            )
            await db.commit()

    async def get_reports_by_number(self, user_id: int, number: str):
        async with aiosqlite.connect(self.db_path) as db:
            cursor = await db.execute(
                """
                SELECT telegram_message_id FROM reports
